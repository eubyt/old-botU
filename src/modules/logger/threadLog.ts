...
async function LoggerThreadRegister(
  client: Client,
    ...
    embeds: [
      {
        ...
        fields: [
          ...
          {
            name: "> Criado em",
            value: `<t:${(data.thread.createdTimestamp ?? 0).toFixed(0)}>`,
          },
        ],
      },
    ],
  ...
  switch (data.event) {
    ...
    case "update":
      ...
      if (data.oldThread?.archived !== data.thread.archived) {
        embed.embeds[0].fields.push(
          {
            name: "Arquivado",
      ...
      if (data.oldThread?.locked !== data.thread.locked) {
        embed.embeds[0].fields.push(
          {
            name: "Trancado",
      ...
      if (
        data.oldThread?.autoArchiveDuration !== data.thread.autoArchiveDuration
      ) {
        embed.embeds[0].fields.push(
          {
            name: "Auto-Arquivar",
      ...
      if (data.oldThread?.rateLimitPerUser !== data.thread.rateLimitPerUser) {
        embed.embeds[0].fields.push(
          {
