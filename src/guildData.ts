...
class GuildData {
  ...
  constructor(guildId: string, public readonly client: Client) {
    ...
    this.documentGuild.name = client.guilds.cache.get(guildId)?.name ?? "Null";
    ...
    db.collection("guilds")
      ...
      .then((doc) => {
        if (!doc.exists) {
          ...
          db.collection("guilds").doc(guildId).set(this.documentGuild);
          console.log(guildId, "not found in database, created!");
        } else {
          // Check old data
          const data = doc.data() as typeof this.documentGuild;
          if (
            Object.keys(data).length !== Object.keys(this.documentGuild).length
          ) {
            ...
            Object.keys(this.documentGuild).forEach((key) => {
              if (data[key as keyof typeof this.documentGuild]) {
                this.documentGuild[key as keyof typeof this.documentGuild] =
  ...
  getLoggerChannels(): {
    [key: string]: PrivateThreadChannel | PublicThreadChannel<boolean> | null;
  } {
